...
class DashBoardProvider extends ChangeNotifier {
  ...
  addProduct() async{
    ...
    try {
      ...
      createFormDataForMultipleImage(imgXFiles: [
          {'image1': mainImgXFile},
...
rethrow;
  ...
  filterSubcategory (Category category) {
    ...
    final newList = _dataProvider.subCategories.where((subcategory) =>
    subcategory.categoryId?.sId == category.sId).toList();
  ...
  filterVariant (VariantType variantType) {
    ...
    variant.variantTypeId?.sId == variantType.sId).toList();
    final List<String> variantNames = newList.map((variant) =>
    variant.name ?? '') .toList();
